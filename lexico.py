import sys  # Importa o módulo sys para utilizar o sys.exit() e encerrar a execução em caso de erro
...
def ler_tokens(nome_arquivo_tokens):
    try:
        ...
        current_section = None  # Variável para rastrear a seção atual no arquivo
        for line in content:
        ...
        return operators, reserved_words, symbols  # Retorna os dicionários preenchidos
    except FileNotFoundError:
...
def lexer(source_code, operators, reserved_words, symbols):
    ...
    while index < len(source_code):
        ...
        if char.isspace():
            if char == '\n':  # Se for nova linha, incrementa o contador de linha
                line_number += 1
                column_number = 0
            index += 1
            column_number += 1
        ...
        if char.isdigit():
            ...
            while index < len(source_code) and (source_code[index].isdigit() or (source_code[index] == '.' and not has_decimal_point)):
                if source_code[index] == '.':
                    has_decimal_point = True  # Define flag para indicar que já tem ponto decimal
                index += 1
